...
func TestClaimsProfileResumeFixtures(t *testing.T) {
	t.Parallel()

	cases := []struct {
		name     string
		input    artifactsv2.ClaimsProjectionInput
		assertFn func(t *testing.T, artifacts artifactsv2.ClaimsProjectionArtifacts)
	}{
		{
			name: "readme_over_claim",
			input: artifactsv2.ClaimsProjectionInput{
		...
		},
		{
			...
			assertFn: func(t *testing.T, artifacts artifactsv2.ClaimsProjectionArtifacts) {
				t.Helper()
				if len(artifacts.Profile.Highlights) != 1 || artifacts.Profile.Highlights[0].SupportLevel != "verified" {
					t.Fatalf("expected verified highlight, got %#v", artifacts.Profile.Highlights)
				}
			},
		},
		{
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			artifacts, err := artifactsv2.BuildClaimsProfileResumeArtifacts(tc.input)
			if err != nil {
				t.Fatalf("BuildClaimsProfileResumeArtifacts(): %v", err)
			}
			if err := artifactsv2.ValidateClaimsProfileResumeArtifacts(artifacts); err != nil {
...
func TestClaimsProfileResumeArtifactsAreWriteable(t *testing.T) {
	t.Parallel()
	...
	dir := t.TempDir()
	if err := artifactsv2.WriteClaimsProfileResumeArtifacts(dir, artifacts); err != nil {
	...
	var profile artifactsv2.ProfileArtifact
